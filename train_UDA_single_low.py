...
def train(hyp, opt, device, tb_writer=None):
    ...
    dis_model = fc_discriminator(num_classes=192).to(device)
    # Resume
    ...
    if opt.sync_bn and cuda and rank != -1:
        ...
        dis_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(dis_model).to(device)
...
###########################################################################################################################
    ...
    #########################################################cyc
    # d_first = get_fc_discriminator(num_classes=192)#192
    # d_first.train()
    # d_first.to(device)
...
################################################################Bisenet的形式
    ...
    if ADD==1:
        d_first_add = get_fc_discriminator_add(num_classes=192)#192
        d_first_add.train()
        d_first_add.to(device)

    if TRAIN_LAYER>1:
        d_second = get_fc_discriminator(num_classes=384)#384
        d_second.train()
        d_second.to(device)

    if TRAIN_LAYER>2:
        d_third = get_fc_discriminator(num_classes=768)#768
        d_third.train()
        d_third.to(device)

    optimizer_d_first = optim.Adam(dis_model.parameters(), lr=TRAIN_LEARNING_RATE_D,
                                 betas=(0.9, 0.99))
    if ADD == 1:
...
###########################################################下面就是按照iter计数
        ...
        for i, (src,trg) in pbar:
            ...
            d_out_first = dis_model(feature_trg[0])#d_out_first:[24,1,2,2]
            ...
            d_out_first = dis_model(feature_src[0])
            if TRAIN_GANLOSS == 'BCE':
            ...
            d_out_first = dis_model(feature_trg[0])
            if TRAIN_GANLOSS == 'BCE':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./runs/train/exp24/weights/best.pt', help='initial weights path')#哪种yolo结构？
    ...
    parser.add_argument('--epochs', type=int, default=25)#几个epoch?
